		...
		for (int i = 0; i < nx; i++) {
			...
			for (int j = 0; j < ny; j++) {
				...
				for (int m = 0; m < nl; m++) {
					...
					UVWP U2 = irregular.uvw2(t0, xpt, ypt, zpt0min);
					UX0[i * ny * nl + j * nl + m] = irregular.u1(t0, xpt, ypt, zpt0min) + U2.u + PHI0_dxdz * zpt0max;
					UY0[i * ny * nl + j * nl + m] = irregular.v1(t0, xpt, ypt, zpt0min) + U2.v + PHI0_dydz * zpt0max;
					UZ0[i * ny * nl + j * nl + m] = irregular.w1(t0, xpt, ypt, zpt0min) + U2.w + PHI0_dzdz * zpt0max;
				}
			}
		}
	}
			...
			for (int i = 0; i < nx; i++) {
				for (int j = 0; j < ny; j++) {
					...
					for (int m = 0; m < nl; m++) {
						...
						UVWP U2 = irregular.uvw2(t0 + dt, xpt, ypt, zpt1min);
						UX1[i * ny * nl + j * nl + m] = irregular.u1(t0 + dt, xpt, ypt, zpt1min) + U2.u + PHI1_dxdz * zpt1max;
						UY1[i * ny * nl + j * nl + m] = irregular.v1(t0 + dt, xpt, ypt, zpt1min) + U2.v + PHI1_dydz * zpt1max;
						UZ1[i * ny * nl + j * nl + m] = irregular.w1(t0 + dt, xpt, ypt, zpt1min) + U2.w + PHI1_dzdz * zpt1max;
					}
				}
			}
		}
