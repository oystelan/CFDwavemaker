...
void Probes::write(double tpt, FentonStream& fenton, Ramp& ramp) {
	// loop over probes and write
	for (int i = 0; i < ts_nprobes; i++) {
		double xpt = coords[i].x;
		double ypt = coords[i].y;
		double zpt = coords[i].z;
		double welev, ux, uy, uz;
		welev = ramp.ramp(tpt, xpt, ypt) * fenton.eta(tpt, xpt, ypt);
		ux = ramp.ramp(tpt, xpt, ypt) * fenton.u(tpt, xpt, ypt, zpt);
		uy = ramp.ramp(tpt, xpt, ypt) * fenton.v(tpt, xpt, ypt, zpt);
		uz = ramp.ramp(tpt, xpt, ypt) * fenton.w(tpt, xpt, ypt, zpt);
...
void Probes::write(double tpt, SpectralWaveData* swd, Ramp& ramp){
	// SWD has stateful API — serialize all access via the same named critical section
	// used elsewhere in the library to avoid races with concurrent SWD calls.
#pragma omp critical(swd_section)
	...
	} // end omp critical(swd_section)
}

// SWD + lsGridSpline probe writer is not available — see lsgrid_spline.cpp note
#endif
