...
for t in time:
    print(wave_VeloY(mydll1,t,x,y,z))
...
mydll2 = dlopen("../../builds/linux64/libCFDwavemaker_all_openmp.so")  
shutil.copyfile("waveinput_swd.dat", 'waveinput.dat')
init_dll(mydll2)
...
# plt.plot(time, wave_elev1, label="cfdwavemaker")
...
# plt.clf()
plt.figure(figsize=(20,6))
plt.plot(time,u1, label='u cfdwavemaker')
#plt.plot(time,v1, label='v cfdwavemaker')
plt.plot(time,w1, label='w cfdwavemaker')
#plt.plot(time,u2,'--', label='u raschii')
#plt.plot(time,v2,'--', label='v raschii')
#plt.plot(time,w2,'--', label='w raschii')
plt.legend()
plt.grid(True)
