...
def wave_DynPres(mydll,t,x,y,z):
    aa = mydll.wave_VeloX
    aa.restype = c_double
    aa.argtypes = [c_double,c_double,c_double,c_double]
    return aa(c_double(x),c_double(y),c_double(z),c_double(t))
...
for t in time:
    ...
    p1.append(wave_DynPres(mydll1,t,x,y,z))
clean_up(mydll1)
dlclose(mydll1)
...
for t in time:
    ...
    p2.append(wave_DynPres(mydll2,t,x,y,z))
clean_up(mydll2)
dlclose(mydll2)


plt.plot(time, wave_elev1, label="linear")
plt.plot(time, wave_elev2,"--", label="second order")
...
plt.show()
plt.clf()
plt.plot(time,u1, label='u linear')
plt.plot(time,v1, label='v linear')
plt.plot(time,w1, label='w linear')
plt.plot(time,u2,'--', label='u second order')
plt.plot(time,v2,'--', label='v second order')
plt.plot(time,w2,'--', label='w second order')
...
plt.clf()
plt.plot(time,p1, label='p linear')
plt.plot(time,p2,'--', label='p second order')
plt.grid(True)
plt.show()
